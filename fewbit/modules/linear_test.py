...
from fewbit.modules.linear import LinearCRS


class TestLinearCRS(TestCase):
    ...
    def test_forward(self):
        for bias in (False, True):
            with self.subTest(bias=bias):
                module = LinearCRS(8, 4, bias)
                module_cloned = self.clone_module(module)
    ...
    def test_backward(self):
        for bias in (False, True):
            with self.subTest(bias=bias):
                ...
                ys, ps, ss = self.estimate_grad(module, xs, 4096)
                zs, qs, ts = self.estimate_grad(module_cloned, xs)
